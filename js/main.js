...
function generateSuggestions(cats) {
  return [
    ...
    { area: 'Monitoring',
      text: 'Schedule quarterly GreenPromise data uploads to track your Sustainability Score over time, set measurable targets, and keep your team accountable to progress milestones.' },
  ];
}
...
function generateTimeline(suggestions) {
  return [
    ...
    },
    {
      ...
      desc:   'Re-upload your updated business data to GreenPromise to measure your improved Sustainability Score. Celebrate progress with your team and set ambitious targets for the year ahead.',
    },
  ];
}
 ...
 *
 * Businesses with a GreenPromise score > 72 are estimated to emit below the
...
async function generatePDF(score, data) {
  ...
  try {
    ...
    const disclaimer = 'This report was generated by GreenPromise based on your uploaded business data. Emissions estimates use established EPA, IPCC, and industry-standard conversion factors. All figures are approximations intended for internal planning purposes.';
    ...
    body(
      `${company} received a GreenPromise Sustainability Score of ${score} out of 100, placing it in the "${getScoreLabel(score)}" tier. ` +
    ...
    gap(2);
    body(
      'Re-upload your business data to GreenPromise at the end of each phase to measure your updated Sustainability Score, ' +
    ...
    body(
      ...
      'Displaying your GreenPromise Sustainability Score publicly communicates genuine, data-backed commitment — not just marketing claims. Businesses that achieve measurable sustainability improvements report stronger customer retention, higher Net Promoter Scores, and greater media visibility.'
    );
    gap(3);
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      doc.setTextColor(...SUBTLE);
      if (i > 1) {
        doc.text(`GreenPromise Sustainability Report  ·  ${company}`, mL, PH - 12);
      }
      doc.text(`${i} / ${totalPages}`, PW - mR, PH - 12, { align: 'right' });
    }

    doc.save('GreenPromise_Sustainability_Report.pdf');
